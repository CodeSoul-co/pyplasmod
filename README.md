...
| 6 | 查 API 细节 | `plasmod_help("easy")` 或阅读 [docs/SDK.md](docs/SDK.md) |
...
已有 **JSON 向量矩阵**（非 `.fbin` 文件）时，可用完整客户端：`p.http.ingest_vectors([[...], [...]])` 或二进制 `p.http.ingest_batch(...)`，详见 [docs/SDK.md](docs/SDK.md)。
...
- **Tier B 扩展 API**：[docs/plans/pyplasmod-002-gateway-tier-b-shortcuts-design.md](docs/plans/pyplasmod-002-gateway-tier-b-shortcuts-design.md)  
