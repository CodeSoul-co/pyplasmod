"""Test cases for AsyncPlasmodClient new features"""

from unittest.mock import ANY, AsyncMock, MagicMock, patch

import pytest
import pytest_asyncio
from pyplasmod import AsyncPlasmodClient, DataType, SearchAggregation, TopHits
from pyplasmod.client.abstract import AnnSearchRequest
from pyplasmod.client.connection_manager import AsyncConnectionManager
...
from pyplasmod.plasmod_client.async_plasmod_client import AsyncPlasmodClientSession
from pyplasmod.orm.collection import Function
...
async def client_and_handler():
    ...
    with patch("pyplasmod.client.async_grpc_handler.AsyncGrpcHandler", return_value=mock_handler):
        client = AsyncPlasmodClient()
        await client._connect()
        yield client, mock_handler


class TestAsyncPlasmodClientNewFeatures:
    """Test cases for newly added features in AsyncPlasmodClient"""
    ...
    def test_cluster_id_not_forwarded_to_handler_on_create(self):
        client = AsyncPlasmodClient(cluster_id="in07-xxx")
    ...
    async def test_list_persistent_segments(self):
        ...
        with patch(
            "pyplasmod.client.async_grpc_handler.AsyncGrpcHandler", return_value=mock_handler
        ):
            # Initialize AsyncPlasmodClient
    ...
    def test_create_struct_field_schema(self):
        ...
        result = AsyncPlasmodClient.create_struct_field_schema()
    ...
    @pytest.mark.asyncio
    async def test_async_plasmod_client_extract_db_name_from_uri(
        self, uri: str, db_name: str, expected_db_name: str
    ):
        """
        Test that AsyncPlasmodClient extracts db_name from URI path when db_name is not explicitly provided.
        ...
        with patch(
            ...
            client = AsyncPlasmodClient(uri=uri, db_name=db_name)
            await client._connect()
...
class TestAsyncPlasmodClientSession:
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
    ...
    async def test_dql_methods_delegate_to_parent_with_cluster_id(self, method_name, args, kwargs):
        ...
        session = AsyncPlasmodClientSession(parent, "in07-xxx")
    ...
    async def test_closed_session_rejects_dql_methods(self):
        session = AsyncPlasmodClientSession(MagicMock(), "in07-xxx")
        await session.close()
...
class TestAsyncPlasmodClientOps:
    """Tests for AsyncPlasmodClient operations with return-value assertions."""
...
# ============================================================
# AsyncPlasmodClient Data Operations Tests
# ============================================================


class TestAsyncPlasmodClientDataOps:
    """Tests for AsyncPlasmodClient data operations."""
...
class TestAsyncPlasmodClientSnapshot:
    """Test snapshot-related APIs in AsyncPlasmodClient."""
...
class TestAsyncPlasmodClientExternalCollection:
    """Test external collection refresh APIs in AsyncPlasmodClient."""
