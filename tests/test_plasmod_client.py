...
import pytest
from pyplasmod import DataType, SearchAggregation, TopHits
...
from pyplasmod.plasmod_client.plasmod_client import PlasmodClient, PlasmodClientSession
from pyplasmod.plasmod_client.optimize_task import OptimizeResult, OptimizeTask
...
class TestPlasmodClient:
    @pytest.mark.parametrize("index_params", [None, {}, "str", PlasmodClient.prepare_index_params()])
    ...
    def test_connection_reuse(self):
        ...
        mock_handler1.get_server_type.return_value = "plasmod"
        mock_handler1._wait_for_channel_ready = MagicMock()

        mock_handler2 = MagicMock()
        mock_handler2.get_server_type.return_value = "plasmod"
        mock_handler2._wait_for_channel_ready = MagicMock()

        mock_handler3 = MagicMock()
        mock_handler3.get_server_type.return_value = "plasmod"
    ...
    def test_add_collection_field_vector_with_nullable_true(self):
        ...
        with patch(
            "pyplasmod.client.grpc_handler.GrpcHandler", return_value=mock_handler
        ), patch.object(PlasmodClient, "_get_connection", return_value=mock_conn):
            client = PlasmodClient()
...
class TestPlasmodClientSnapshot:
    """Test snapshot-related APIs in PlasmodClient."""
    ...
    def test_client_db_isolation(self):
        ...
        with patch("pyplasmod.client.grpc_handler.GrpcHandler", return_value=mock_handler):
            client_a = PlasmodClient(uri="http://localhost:19530", db_name="default")
            client_b = PlasmodClient(uri="http://localhost:19530", db_name="testdb")
    ...
    def test_plasmod_client_extract_db_name_from_uri(
        self, uri: str, db_name: str, expected_db_name: str
    ):
        """
        Test that PlasmodClient extracts db_name from URI path when db_name is not explicitly provided.
        ...
        with patch("pyplasmod.client.grpc_handler.GrpcHandler", return_value=mock_handler):
            client = PlasmodClient(uri=uri, db_name=db_name)
...
def mc():
    ...
    with patch("pyplasmod.client.grpc_handler.GrpcHandler", return_value=handler):
        yield PlasmodClient(), handler


class TestPlasmodClientCreateCollection:
...
class TestPlasmodClientCollectionDetails:
...
class TestPlasmodClientOptimize:
...
class TestPlasmodClientSearchOps:
    ...
    def test_cluster_id_not_forwarded_to_handler_on_create(self):
        handler = _make_handler()
        with patch("pyplasmod.client.grpc_handler.GrpcHandler", return_value=handler) as mock_cls:
            PlasmodClient(cluster_id="in07-xxx")
    ...
    def test_session_close_does_not_close_parent(self):
        ...
        with patch("pyplasmod.client.grpc_handler.GrpcHandler", return_value=handler):
            ...
            with pytest.raises(PlasmodException, match="session is closed"):
                search_cluster.search("col", data=[[0.1, 0.2]])
    ...
    def test_query_iterator_delegates(self):
        ...
        with patch("pyplasmod.client.grpc_handler.GrpcHandler", return_value=handler):
            client = PlasmodClient()
            with patch("pyplasmod.plasmod_client.plasmod_client.QueryIterator") as mock_qi:
    ...
    def test_search_iterator_delegates_v2(self):
        ...
        with patch("pyplasmod.client.grpc_handler.GrpcHandler", return_value=handler):
            client = PlasmodClient()
            with patch("pyplasmod.plasmod_client.plasmod_client.SearchIteratorV2") as mock_v2:
                mock_v2.return_value = MagicMock()
                client.search_iterator("col", data=[[0.1, 0.2]])
                mock_v2.assert_called_once()


class TestPlasmodClientSession:
    @pytest.mark.parametrize(
    ...
    def test_dql_methods_delegate_to_parent_with_cluster_id(self, method_name, args, kwargs):
        ...
        session = PlasmodClientSession(parent, "in07-xxx")
    ...
    def test_closed_session_rejects_dql_methods(self):
        session = PlasmodClientSession(MagicMock(), "in07-xxx")
        session.close()
...
class TestPlasmodClientDeleteBranches:
...
class TestPlasmodClientSimpleDelegation:
    @pytest.mark.parametrize(
...
class TestPlasmodClientCollectionMgmt:
    ...
    def test_get_load_state_exception_reraises(self):
        handler = _make_handler()
        handler.get_load_state.side_effect = PlasmodException(message="load error")
        with patch("pyplasmod.client.grpc_handler.GrpcHandler", return_value=handler):
            client = PlasmodClient()
...
class TestPlasmodClientMiscOps:
...
class TestPlasmodClientRBACOps:
    ...
    def test_describe_user_exception_reraises(self, mc):
        client, handler = mc
        handler.select_one_user.side_effect = PlasmodException(message="not found")
        with pytest.raises(PlasmodException):
    ...
    def test_describe_role_exception_reraises(self, mc):
        client, handler = mc
        handler.select_grant_for_one_role.side_effect = PlasmodException(message="err")
        with pytest.raises(PlasmodException):
    ...
    def test_list_roles_exception_reraises(self, mc):
        client, handler = mc
        handler.select_all_role.side_effect = PlasmodException(message="err")
        with pytest.raises(PlasmodException):
            client.list_roles()
...
class TestPlasmodClientGetPartitionStats:
...
class TestPlasmodClientInternalOps:
    ...
    def test_wait_for_compaction_failed_raises(self, mc):
        ...
        task = MagicMock()
        task.check_cancelled = MagicMock()
        with pytest.raises(PlasmodException, match="Compaction 42 failed"):
            client._wait_for_compaction_with_cancel(task, 42)
...
class TestFileResourceMethods(TestPlasmodClient):
...
class TestPlasmodClientExternalCollection:
    """Test external collection refresh APIs in PlasmodClient."""
