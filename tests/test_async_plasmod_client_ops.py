...
from pyplasmod.exceptions import DataTypeNotMatchException, PlasmodException, ParamError
from pyplasmod.plasmod_client.async_plasmod_client import AsyncPlasmodClient
from pyplasmod.plasmod_client.async_optimize_task import AsyncOptimizeTask
from pyplasmod.plasmod_client.optimize_task import OptimizeResult
...
class TestAsyncClientCreateCollection:
    ...
    async def test_create_collection_with_schema_no_index_params(self):
        client, handler = _make_client()
        schema = AsyncPlasmodClient.create_schema()
    ...
    async def test_create_collection_with_schema_with_index_params(self):
        ...
        schema.add_field("id", DataType.INT64, is_primary=True)
        schema.add_field("vec", DataType.FLOAT_VECTOR, dim=4)
        ip = AsyncPlasmodClient.prepare_index_params("vec", index_type="FLAT", metric_type="L2")
        with patch.object(client, "create_index"), patch.object(client, "load_collection"):
...
class TestAsyncClientIndexOps:
    ...
    async def test_create_index_empty_raises(self):
        client, _ = _make_client()
        ip = AsyncPlasmodClient.prepare_index_params()
...
class TestAsyncClientRBACOps:
    ...
    async def test_describe_user_with_groups(self):
        ...
        with patch("pyplasmod.plasmod_client.async_plasmod_client.UserInfo") as mock_ui:
    ...
    async def test_list_roles(self):
        client, handler = _make_client()
        with patch("pyplasmod.plasmod_client.async_plasmod_client.RoleInfo") as mock_ri:
            g = MagicMock()
            g.role_name = "admin"
