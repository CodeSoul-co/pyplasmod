    ...
    def inner(*args, **kwargs):
        LOGGER.warning(
            WARNING_COLOR.format(
                "[WARNING] PyPlasmod: ",
                "class Plasmod will be deprecated soon, please use Collection/utility instead",
            )
        )
                ...
                while True:
                    try:
                        if retry_timeout is not None:
                            ...
                            if remaining <= 0:
                                raise PlasmodException(message=to_msg)
                        ...
                        raise PlasmodException(message=to_msg) from e
        ...
        if inspect.iscoroutinefunction(func):
            ...
            async def async_handler(*args, **kwargs):
                ...
                try:
                    ...
                    raise PlasmodException(message=f"Unexpected error, message=<{e!s}>") from e
        ...
        try:
            ...
            if e.code() == grpc.StatusCode.UNIMPLEMENTED:
                msg = (
                    ...
                    "Please first verify that your uri points to a Plasmod gRPC port "
                    "(default: 19530) instead of an HTTP/proxy port. "
                    "If the port is correct, your SDK may be incompatible with the server; "
                    "upgrade Plasmod server or use a matching pyplasmod version."
                )
                raise PlasmodException(message=msg) from e
            raise e from e
        except Exception as e:
            raise e from e
