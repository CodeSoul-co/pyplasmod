"""Connection manager for PlasmodClient.

This module provides connection pooling, lifecycle management, and strategy-based
connection handling for PlasmodClient, bypassing the legacy connections singleton.
"""
...
class ConnectionConfig:
    """Configuration for a Plasmod connection.
    ...
    def from_uri(
        ...
        **kwargs,
        ...
        """
        # --- Local *.db URI: optional embedded engine rewrites to a gRPC endpoint ---
        if uri.endswith(".db"):
            parent = pathlib.Path(uri).parent
            if not parent.is_dir():
                raise ConnectionConfigException(
                    message=f"Open local database failed, dir: {parent} not exists"
                )
            try:
                ...
                raise ConnectionConfigException(
                    ...
                    "Install the vendor wheel that provides the embedded server module."
                ) from e
...
class ManagedConnection:
    ...
    Attributes:
        ...
        clients: WeakSet of PlasmodClient instances using this connection
    """
...
class RegularStrategy(ConnectionStrategy):
    ...
    Creates direct connections to a single Plasmod endpoint.
    Always triggers recovery on UNAVAILABLE errors.
    """

    def create_handler(self, config: ConnectionConfig) -> "GrpcHandler":
        """Create a GrpcHandler for the configured endpoint."""
        from pyplasmod.client.grpc_handler import GrpcHandler  # noqa: PLC0415
...
class ConnectionManager:
    """Manages connections for PlasmodClient.
    ...
    def get_or_create(
        ...
        Args:
            ...
            client: PlasmodClient instance to track (for shared connections)
    ...
    def _register_error_callback(self, handler: "GrpcHandler") -> None:
        ...
        on gRPC errors and retryable PlasmodExceptions. This routes errors
    ...
    def _recover(self, managed: ManagedConnection) -> None:
        ...
        channel) so that all existing references — PlasmodClient._handler and
    ...
    def handle_error(
        ...
        For STREAMING_CODE_REPLICATE_VIOLATION (PlasmodException), triggers
        ...
        if isinstance(error, grpc.RpcError):
            if error.code() != grpc.StatusCode.UNAVAILABLE:
                return False
        elif isinstance(error, PlasmodException):
...
class AsyncRegularStrategy(ConnectionStrategy):
    ...
    def create_handler(self, config: ConnectionConfig) -> "AsyncGrpcHandler":
        ...
        from pyplasmod.client.async_grpc_handler import AsyncGrpcHandler  # noqa: PLC0415
...
class AsyncConnectionManager:
    """Async connection manager for AsyncPlasmodClient.
    ...
    def _register_error_callback(self, handler: "AsyncGrpcHandler") -> None:
        ...
        on gRPC errors and retryable PlasmodExceptions. This routes errors to
    ...
    async def _recover(self, managed: ManagedConnection) -> None:
        ...
        Reuses the existing handler object (closing and recreating its gRPC
        channel) so that all existing references — AsyncPlasmodClient._handler
