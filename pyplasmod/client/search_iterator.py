...
from pyplasmod.client.constants import (
    COLLECTION_ID,
    GUARANTEE_TIMESTAMP,
...
from pyplasmod.client.search_result import Hit, Hits
from pyplasmod.exceptions import ExceptionsMessage, ParamError, ServerVersionIncompatibleException
from pyplasmod.orm.connections import Connections
from pyplasmod.orm.constants import MAX_BATCH_SIZE, OFFSET, UNLIMITED
from pyplasmod.orm.iterator import SearchPage, fall_back_to_latest_session_ts
...
class SearchIteratorV2:
    ...
    def _probe_for_compability(self, params: Dict):
        ...
        # and produce duplicate PKs. See: https://github.com/plasmod-io/pyplasmod/issues/3421
    ...
    def _next(self):
        ...
        if self._params[GUARANTEE_TIMESTAMP] <= 0:
            if res.get_session_ts() > 0:
                ...
                logger.warning(
                    "failed to set up mvccTs from plasmod server, use client-side ts instead"
                )
