...
from pyplasmod.exceptions import PlasmodException, ParamError, SchemaMismatchRetryableException
...
def mkts_from_hybridts(
    ...
    if not isinstance(milliseconds, (int, float)):
        raise PlasmodException(message="parameter milliseconds should be type of int or float")

    if isinstance(delta, datetime.timedelta):
        milliseconds += delta.microseconds / 1000.0
    elif delta is not None:
        raise PlasmodException(message="parameter delta should be type of datetime.timedelta")

    if not isinstance(hybridts, int):
        raise PlasmodException(message="parameter hybridts should be type of int")
...
def mkts_from_unixtime(
    ...
    if not isinstance(epoch, (int, float)):
        raise PlasmodException(message="parameter epoch should be type of int or float")
...
def mkts_from_datetime(
    ...
    if not isinstance(d_time, datetime.datetime):
        raise PlasmodException(message="parameter d_time should be type of datetime.datetime")
...
def len_of(field_data: Any) -> int:
    ...
    if field_kind == "scalars":
        ...
        if scalar_kind is None:
            raise PlasmodException(message="Unsupported scalar type")
    ...
    raise PlasmodException(message="Unknown data type")
...
def get_params(search_params: Dict):
    ...
    # to ensure compatibility and plasmod can still get a json format parameter
...
def get_server_type(host: str):
    return ZILLIZ if (isinstance(host, str) and "zilliz" in host.lower()) else DEFAULT_CLUSTER_KIND
