...
from pyplasmod.grpc_gen import common_pb2, rg_pb2, schema_pb2
...
class Group:
    """
    This class represents replica info in orm format api, which is deprecated in plasmod client api.
...
class Replica:
    ...
    which is deprecated in plasmod client api.
...
class GrantInfo:
    ...
    def __init__(self, entities: List[plasmod_types.RoleEntity]) -> None:
        groups = []
        for entity in entities:
            if isinstance(entity, plasmod_types.GrantEntity):
...
class PrivilegeGroupItem:
    def __init__(self, privilege_group: str, privileges: List[plasmod_types.PrivilegeEntity]):
        self._privilege_group = privilege_group
        privielges = []
        for privilege in privileges:
            if isinstance(privilege, plasmod_types.PrivilegeEntity):
...
class PrivilegeGroupInfo:
    ...
    def __init__(self, results: List[plasmod_types.PrivilegeGroupInfo]) -> None:
        groups = []
        for result in results:
            if isinstance(result, plasmod_types.PrivilegeGroupInfo):
...
class UserItem:
    def __init__(self, username: str, entities: List[plasmod_types.RoleEntity]) -> None:
        self._username = username
        roles = []
        for entity in entities:
            if isinstance(entity, plasmod_types.RoleEntity):
...
class UserInfo:
    ...
    def __init__(self, results: List[plasmod_types.UserResult]):
        groups = []
        for result in results:
            if isinstance(result, plasmod_types.UserResult):
...
class RoleItem:
    def __init__(self, role_name: str, entities: List[plasmod_types.UserEntity]):
        self._role_name = role_name
        users = []
        for entity in entities:
            if isinstance(entity, plasmod_types.UserEntity):
...
class RoleInfo:
    ...
    def __init__(self, results: List[plasmod_types.RoleResult]) -> None:
        groups = []
        for result in results:
            if isinstance(result, plasmod_types.RoleResult):
...
class AnalyzeToken:
    def __init__(
        self, token: plasmod_types.AnalyzerToken, with_hash: bool = False, with_detail: bool = False
    ):
...
class AnalyzeResult:
    def __init__(
        self, info: plasmod_types.AnalyzerResult, with_hash: bool = False, with_detail: bool = False
    ) -> None:
...
class FileResourceInfo:
    def __init__(self, info: plasmod_types.FileResourceInfo) -> None:
        self.name = info.name
        self.path = info.path
...
def parse_refresh_job_info(
    info: "plasmod_types.RefreshExternalCollectionJobInfo",
    ...
    return RefreshExternalCollectionJobInfo(
        ...
        state=plasmod_types.RefreshExternalCollectionState.Name(info.state),
