...
def fetch_topology(global_endpoint: str, token: str) -> GlobalTopology:
    ...
    Raises:
        PlasmodException: If topology cannot be fetched after retries
    """
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if result.get("code", 0) != 0:
                raise PlasmodException(message=result.get("message", "Unknown API error"))

            return _parse_topology_response(result["data"])

        except PlasmodException:
            raise
