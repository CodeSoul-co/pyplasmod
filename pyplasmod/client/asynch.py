...
from pyplasmod.grpc_gen import plasmod_pb2
...
def _build_none_response_exception(future: Any) -> PlasmodException:
    """Build a PlasmodException when a gRPC future returns None as the response message.
    ...
    try:
        ...
        if code == grpc.StatusCode.DEADLINE_EXCEEDED:
            return PlasmodException(message=f"gRPC call timed out (DEADLINE_EXCEEDED): {details}")
        return PlasmodException(
            message=f"Received None response from server (code={code.name}, details={details})"
        )
    except Exception:
        return PlasmodException(message="Received None response from server")
            ...
            for cb in self._done_cb_list:
                if cb:
                    ...
                    if isinstance(self._results, tuple):
                        ...
                        raise PlasmodException(message="callback function is not legal!")
        ...
        with self._condition:
            ...
            if self._future and self._results is None:
                try:
                    ...
                    raise PlasmodException(message=str(e)) from e
            ...
            if self._future and self._results is None:
                try:
                    ...
                    if self._response is None:
                        ...
                        self._results = self.on_response(self._response)
                        self._callback()  # https://github.com/plasmod-io/plasmod/issues/6160
                except Exception as e:
                    self._exception = e
...
class SearchFuture(Future):
    def on_response(self, response: plasmod_pb2.SearchResults):
        if response is None:
