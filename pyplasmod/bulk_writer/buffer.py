        ...
        for k, v in self._buffer.items():
            ...
            try:
                ...
                if field_schema.dtype == DataType.ARRAY:
                    # currently, plasmod server doesn't support numpy for array field
                    self._throw(
                        f"Failed to persist file {full_file_name},"
                        f" error: plasmod doesn't support parsing array type values from numpy file"
                    )
                    ...
                    # currently, plasmod server doesn't support numpy for sparse vector
                    self._throw(
                        f"Failed to persist file {full_file_name},"
                        f" error: plasmod doesn't support parsing sparse vectors from numpy file"
                    )
