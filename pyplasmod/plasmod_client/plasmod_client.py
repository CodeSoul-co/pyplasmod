...
from pyplasmod.client.abstract import AnnSearchRequest, BaseRanker
from pyplasmod.client.connection_manager import ConnectionConfig, ConnectionManager
from pyplasmod.client.constants import CLUSTER_ID, DEFAULT_CONSISTENCY_LEVEL
from pyplasmod.client.embedding_list import EmbeddingList
from pyplasmod.client.search_aggregation import SearchAggregation
...
from pyplasmod.orm.collection import CollectionSchema, Function, FunctionScore, Highlighter
from pyplasmod.orm.constants import FIELDS, METRIC_TYPE, TYPE, UNLIMITED
from pyplasmod.orm.iterator import QueryIterator, SearchIterator
from pyplasmod.orm.types import DataType

from .base import BasePlasmodClient
...
class PlasmodClient(BasePlasmodClient):
    """The Plasmod Client"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """A client for the common Plasmod use case.

        This client attempts to hide away the complexity of the underlying RPC stack. In a lot of cases what
        ...
        self.is_self_hosted = bool(self.get_server_type() == "plasmod")

    def session(self, cluster_id: str) -> "PlasmodClientSession":
        """Create a lightweight client session pinned to a target cluster."""
        validate_param("cluster_id", cluster_id, str)
        return PlasmodClientSession(self, cluster_id)
    ...
    def insert(
        ...
        **kwargs,
        ...
        If the Plasmod Client was initiated without an existing Collection, the first dict passed
        ...
        Raises:
            ...
            PlasmodException: General Plasmod error on insert.
    ...
    def upsert(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs (dict): Extra keyword arguments.
                ...
                * *field_ops* (Dict[str, Any], optional): Per-field partial-update operators that
                    ...
                    value may be a :class:`~pyplasmod.FieldOp` factory result (e.g.
        ...
        Raises:
            DataNotMatchException: If the data has missing fields an exception will be thrown.
            PlasmodException: General Plasmod error on upsert.
        ...
        return OmitZeroDict(
            {
                ...
                "cost": res.cost,
                # plasmod server supports upsert on autoid=ture from v2.4.15
    ...
    def search_iterator(
        ...
        **kwargs,
        ...
        Raises:
            PlasmodException: If the search operation fails.
        ...
        if anns_field is None or anns_field == "":
            ...
            if vec_field is None:
                raise PlasmodException(
                    ...
                    message="there should be at least one vector field in plasmod collection",
                )
    ...
    def delete(
        ...
        **kwargs,
        ...
        Starting from version 2.3.2, Plasmod no longer includes the primary keys in the result
        ...
        Plasmod(previous 2.3.2) is not empty, the list of primary keys is still returned.
    ...
    def _get_connection(self):
        ...
        if self._handler is None:
            raise PlasmodException(message="should create connection first")
    ...
    def use_database(self, db_name: str, **kwargs):
        ...
        Raises:
            PlasmodException: If the database does not exist (error code 800).
        """
    ...
    def update_replicate_configuration(
        ...
        **kwargs,
        ...
        """
        Update replication configuration across Plasmod clusters.
        ...
        Raises:
            ...
            PlasmodException: If the operation fails
    ...
    def get_replicate_configuration(
        ...
        **kwargs,
        ...
        """
        Get replication configuration from Plasmod.
    ...
    def _wait_for_compaction_with_cancel(
        ...
        while True:
            ...
            if state.state == 2:
                break
            if state.state == 3:
                raise PlasmodException(message=f"Compaction {compaction_id} failed")
    ...
    def optimize(
        ...
        **kwargs,
        ...
        Raises:
            ...
            PlasmodException: If index build fails, compaction fails, or timeout occurs.
...
class PlasmodClientSession:
    """DQL session bound to a cluster id and backed by a parent PlasmodClient."""

    def __init__(self, parent: PlasmodClient, cluster_id: str) -> None:
        self._parent = parent
    ...
    def _ensure_open(self) -> None:
        if self._closed:
            raise PlasmodException(message="PlasmodClient session is closed")
