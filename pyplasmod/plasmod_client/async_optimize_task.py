...
class AsyncOptimizeTask:
    ...
    def start(self) -> None:
        async def _run() -> OptimizeResult:
            try:
                ...
                raise PlasmodException(message="Optimization task was cancelled") from e
            else:
                return result
    ...
    def check_cancelled(self) -> None:
        if self._cancelled:
            raise PlasmodException(message="Optimization task was cancelled")

    async def result(self, timeout: Optional[float] = None) -> OptimizeResult:
        if not self._task:
            raise PlasmodException(message="Task has not been started")
        ...
        try:
            ...
            raise PlasmodException(message="Timeout waiting for optimization to complete") from e
        except asyncio.CancelledError as e:
            raise PlasmodException(message="Optimization task was cancelled") from e
