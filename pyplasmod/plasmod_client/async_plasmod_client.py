...
from pyplasmod.client.connection_manager import AsyncConnectionManager, ConnectionConfig
...
from pyplasmod.orm.collection import CollectionSchema, Function, FunctionScore
...
class AsyncPlasmodClient(BasePlasmodClient):
    """AsyncPlasmodClient is an EXPERIMENTAL class
    which only provides part of PlasmodClient's methods"""
    ...
    def session(self, cluster_id: str) -> "AsyncPlasmodClientSession":
        """Create a lightweight client session pinned to a target cluster."""
        validate_param("cluster_id", cluster_id, str)
        return AsyncPlasmodClientSession(self, cluster_id)
    ...
    async def _connect(self) -> None:
        ...
        self.is_self_hosted = bool(self._handler.get_server_type() == "plasmod")
...
class AsyncPlasmodClientSession:
    """Async DQL session bound to a cluster id and backed by a parent client."""

    def __init__(self, parent: AsyncPlasmodClient, cluster_id: str) -> None:
        self._parent = parent
    ...
    def _ensure_open(self) -> None:
        if self._closed:
            raise PlasmodException(message="AsyncPlasmodClient session is closed")
