...
class CollectionNotExistException(PlasmodException):
    """Raise when collections doesn't exist"""


class DescribeCollectionException(PlasmodException):
    """Raise when fail to describe collection"""


class PartitionAlreadyExistException(PlasmodException):
    """Raise when create an exsiting partition"""


class IndexNotExistException(PlasmodException):
    """Raise when index doesn't exist"""


class AmbiguousIndexName(PlasmodException):
    """Raise multiple index exist, need specify index_name"""


class CannotInferSchemaException(PlasmodException):
    """Raise when cannot trasfer dataframe to schema"""


class SchemaNotReadyException(PlasmodException):
    """Raise when schema is wrong"""


class DataTypeNotMatchException(PlasmodException):
    """Raise when datatype dosen't match"""


class DataTypeNotSupportException(PlasmodException):
    """Raise when datatype isn't supported"""


class DataNotMatchException(PlasmodException):
    """Raise when insert data isn't match with schema"""


class SchemaMismatchRetryableException(PlasmodException):
...
class ConnectionNotExistException(PlasmodException):
    """Raise when connections doesn't exist"""


class ConnectionConfigException(PlasmodException):
    """Raise when configs of connection are invalid"""


class PrimaryKeyException(PlasmodException):
    """Raise when primarykey are invalid"""


class PartitionKeyException(PlasmodException):
    """Raise when partitionkey are invalid"""


class ClusteringKeyException(PlasmodException):
    """Raise when clusteringkey are invalid"""


class FieldsTypeException(PlasmodException):
    """Raise when fields is invalid"""


class FunctionsTypeException(PlasmodException):
    """Raise when functions are invalid"""


class FieldTypeException(PlasmodException):
    """Raise when one field is invalid"""


class AutoIDException(PlasmodException):
    """Raise when autoID is invalid"""


class InvalidConsistencyLevel(PlasmodException):
    """Raise when consistency level is invalid"""


class ServerVersionIncompatibleException(PlasmodException):
...
class ExceptionsMessage:
    ...
    CollectionType = "The type of collection must be pyplasmod.Collection."
    ...
    Please upgrade your Plasmod server version to 2.5.2 and later,
    or use a pyplasmod version before 2.5.3 (excluded) to avoid this issue.
    """
