...
from pyplasmod.client import entity_helper, utils
from pyplasmod.client.abstract import LoopBase
from pyplasmod.client.search_result import Hits
...
class QueryIteratorCursor:
    ...
    def to_proto(self) -> plasmod_types.QueryCursor:
        cursor = plasmod_types.QueryCursor()
...
def assert_info(condition: bool, message: str):
    if not condition:
        raise PlasmodException(message)
...
def io_operation(io_func: Callable[[Any], None], message: str):
    try:
        ...
        raise PlasmodException(message=message) from ose
...
class QueryIterator:
    ...
    def __seek_to_offset(self):
        ...
        if offset > 0:
            ...
            def seek_offset_by_batch(batch: int, expr: str) -> int:
                seek_params[PLASMOD_LIMIT] = batch
    ...
    def __check_set_batch_size(self, batch_size: int):
        ...
        self._kwargs[PLASMOD_LIMIT] = batch_size
    ...
    def __setup_ts_by_request(self):
        ...
        init_ts_kwargs[PLASMOD_LIMIT] = 1
        ...
        if res is None:
            raise PlasmodException(
                message="failed to connect to plasmod for setting up "
                "mvccTs, check plasmod servers' status"
            )
        if res.extra is not None:
            self._session_ts = res.extra.get(ITERATOR_SESSION_TS_FIELD, 0)
        if self._session_ts <= 0:
            log.warning("failed to get mvccTs from plasmod server, use client-side ts instead")
    ...
    def __setup__pk_prop(self):
        ...
        if self._pk_field_name is None or self._pk_field_name == "":
            raise PlasmodException(message="schema must contain pk field, broke")
...
def metrics_positive_related(metrics: str) -> bool:
    ...
    if metrics in [CALC_DIST_IP, CALC_DIST_COSINE, CALC_DIST_BM25]:
        return False
    raise PlasmodException(message=f"unsupported metrics type for search iteration: {metrics}")
...
class SearchIterator:
    ...
    def __init_search_iterator(self):
        ...
        if self._session_ts <= 0:
            log.warning("failed to set up mvccTs from plasmod server, use client-side ts instead")
            self._session_ts = fall_back_to_latest_session_ts()
        self._kwargs[GUARANTEE_TIMESTAMP] = self._session_ts
        if len(init_page) == 0:
            message = (
