...
from pyplasmod.client.check import is_legal_address, is_legal_host, is_legal_port
...
class Connections(metaclass=SingleInstanceMetaClass):
    """Class for managing all connections to the vector database (Plasmod / Plasmod-compatible gRPC)."""

    def __init__(self) -> None:
        """Constructs a default connection alias config.

            Default config is read from env ``PLASMOD_URI`` and ``PLASMOD_CONN_ALIAS``,
            with default address ``localhost:19530`` when no URI is set.
        ...
        if Config.PLASMOD_URI != "":
            address, parsed_uri = self.__parse_address_from_uri(Config.PLASMOD_URI)
        ...
        self.add_connection(**{Config.PLASMOD_CONN_ALIAS: default_conn_config})
    ...
    def add_connection(self, **kwargs):
        """Configures a plasmod connection.

        Addresses priority in kwargs: address, uri, host and port

        :param kwargs:
            * *address* (``str``) -- Optional. The actual address of Plasmod instance.
                Example address: "localhost:19530"

            * *uri* (``str``) -- Optional. The uri of Plasmod instance.
                Example uri: "http://localhost:19530", "tcp:localhost:19530", "https://ok.s3.south.com:19530".

            * *host* (``str``) -- Optional. The host of Plasmod instance.
                Default at "localhost", PyPlasmod will fill in the default host
                if only port is provided.

            * *port* (``str/int``) -- Optional. The port of Plasmod instance.
                Default at 19530, PyPlasmod will fill in the default port if only host is provided.
    ...
    def disconnect(self, alias: str):
        ...
        :param alias: The name of plasmod connection
        :type alias: str
        """
    ...
    def connect(
        self,
        alias: str = Config.PLASMOD_CONN_ALIAS,
        user: str = "",
        password: str = "",
        db_name: str = "default",
        token: str = "",
        ...
        **kwargs,
        ...
        """Constructs a plasmod connection and register it under given alias.

        Args:
            alias (str): Default to "default". The name of connection. Each alias corresponds to one
                connection.
            user (str, Optional): The user of plasmod server.
            password (str, Optional): The password of plasmod server.
            token (str, Optional): Serving as the key for authentication.
            db_name (str): The database name of plasmod server.
            ...
            _unbind_with_db (bool, Optional): Whether the alias should be bound with a database.
                ...
                - If True (for PlasmodClient/AsyncPlasmodClient):
            ...
            **kwargs:
                ...
                * uri (str, Recommanded) -- The uri of Plasmod instance.
                   Example uri: "http://localhost:19530", "tcp:localhost:19530", "https://ok.s3.south.com:19530".
                * host (str, Optional) -- The host of Plasmod instance. Default at "localhost",
                    PyPlasmod will fill in the default host if only port is provided.
                * port (str/int, Optional) -- The port of Plasmod instance. Default at 19530,
                    PyPlasmod will fill in the default port if only host is provided.
        ...
        Example:
            >>> from pyplasmod import connections
        ...
        Raises:
            ...
            PlasmodException: If anything goes wrong.
        ...
        if kwargs.get("uri") and parse.urlparse(kwargs["uri"]).scheme.lower() not in [
            ...
            # optional embedded engine for *.db paths
            ...
            logger.info("Starting optional local engine for path %s", kwargs["uri"])
            parent_path = pathlib.Path(kwargs["uri"]).parent
            if not parent_path.is_dir():
                raise ConnectionConfigException(
                    message=f"Open local database failed, dir: {parent_path} not exists"
                )

            try:
                smod = importlib.import_module(f"{lite_embedding_package()}.server_manager")
                server_manager_instance = smod.server_manager_instance
            except ImportError as e:
                raise ConnectionConfigException(
                    message="Optional local engine package is required for .db URIs."
                ) from e

            local_uri = server_manager_instance.start_and_get_uri(kwargs["uri"])
            if local_uri is None:
                raise ConnectionConfigException(message="Open local database failed")
        ...
        def connect_grpc_handler(**kwargs):
            ...
            timeout = t if isinstance(t, (int, float)) else Config.PLASMOD_CONN_TIMEOUT
        ...
        if with_config(config):
            ...
            connect_grpc_handler(**kwargs, user=user, password=password, token=token, db_name=db_name)
            return
        ...
        if self._env_uri is not None:
            ...
            connect_grpc_handler(**kwargs, user=user, password=password, db_name=db_name)
            return
        ...
        if alias in self._alias_config:
            ...
            connect_grpc_handler(**connect_alias, password=password, db_name=db_name, **kwargs)
            return
    ...
    def _update_db_name(self, alias: str, db_name: str) -> None:
        ...
        It cannot be used for aliases created by
         PlasmodClient/AsyncPlasmodClient (unbound), which
    ...
    def _fetch_handler(
        self, alias: str = Config.PLASMOD_CONN_ALIAS
    ...
    def _generate_call_context(self, alias: str, **kwargs) -> CallContext:
        ...
        Args:
            alias (str): The name of plasmod connection
            **kwargs:
