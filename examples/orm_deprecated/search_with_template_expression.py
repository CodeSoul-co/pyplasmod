...
for expr, expr_params in exprs.items():
    ...
    result = hello_plasmod.search(vectors_to_search, "embeddings", search_params, limit=3, expr=expr,
    ...
    # query based on scalar filtering(boolean, int, etc.)
    start_time = time.time()
    result = hello_plasmod.query(expr=expr, output_fields=["random", "embeddings"], expr_params=expr_params)
    end_time = time.time()
    ...
    # -----------------------------------------------------------------------------
    # pagination
    r1 = hello_plasmod.query(expr=expr, limit=4, output_fields=["random"], expr_params=expr_params)
    r2 = hello_plasmod.query(expr=expr, offset=1, limit=3, output_fields=["random"], expr_params=expr_params)
...
hello_plasmod.delete(expr, expr_params=expr_params)
