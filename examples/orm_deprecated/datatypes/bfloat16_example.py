...
def bf16_vector_search():
    ...
    if utility.has_collection("hello_plasmod_fp16"):
        utility.drop_collection("hello_plasmod_fp16")
    hello_plasmod = Collection("hello_plasmod_fp16", schema, consistency_level="Strong")

    _, vectors = gen_bf16_vectors(nb, dim)
    hello_plasmod.insert([vectors[:6]])
    rows = [
    ...
    for i, index_type in enumerate(bf16_index_types):
        ...
        res = hello_plasmod.search(vectors[0:10], vector_field_name, {"metric_type": "L2"}, limit=1, output_fields=["bfloat16_vector"])
