...
schema = CollectionSchema(fields, "hello_plasmod is the simplest demo to introduce the APIs", enable_dynamic_field=True)
...
################################################################################
# 3. insert data
hello_plasmod2 = Collection("hello_plasmod")
...
insert_result = hello_plasmod.insert(rows)

hello_plasmod.insert({"pk": "7", "random": 1.0, "embeddings": rng.random((1, dim))[0], "g": 1})
hello_plasmod.flush()
print(f"Number of entities in Plasmod: {hello_plasmod.num_entities}")  # check the num_entites
...
result = hello_plasmod.search(vectors_to_search, "embeddings", search_params, limit=3, output_fields=["pk", "embeddings"])
end_time = time.time()
...
result = hello_plasmod.search(vectors_to_search, "embeddings", search_params, limit=3, output_fields=["pk", "embeddings", "$meta"])
for hits in result:
    for hit in hits:
        print(f"hit: {hit}")
...
result = hello_plasmod.query(expr=expr, output_fields=["random", "a", "g"])
for hit in result:
    print("hit:", hit)
