...
plasmod_client.alter_index_properties(collection_name,index_name="embeddings", properties={"mmap.enabled":True})
plasmod_client.drop_index_properties(collection_name,index_name="embeddings", property_keys=["mmap.enabled"])
plasmod_client.alter_collection_field(collection_name,field_name="title",field_params={"mmap.enabled":True,"max_length": 2500})
plasmod_client.alter_collection_properties(collection_name, properties={"mmap.enabled":True,"collection.ttl.seconds": 500})
plasmod_client.drop_collection_properties(collection_name,index_name="embeddings", property_keys=["mmap.enabled"])
plasmod_client.load_collection(collection_name)
print(plasmod_client.describe_index(collection_name,index_name="embeddings"))
print(plasmod_client.describe_collection(collection_name))
plasmod_client.release_collection(collection_name)
plasmod_client.drop_index(collection_name, "embeddings")
plasmod_client.drop_collection(collection_name)
