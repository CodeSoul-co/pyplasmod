...
index_names = plasmod_client.list_indexes(collection_name)
print(f"index names for {collection_name}:", index_names)
for index_name in index_names:
    index_info = plasmod_client.describe_index(collection_name, index_name=index_name)
...
query_results = plasmod_client.query(collection_name, filter= "f == 600 or title == 't2'")
for ret in query_results:
    print(ret)

vectors_to_search = rng.random((1, dim))
print(fmt.format(f"Start search with retrieve several fields."))
result = plasmod_client.search(collection_name, vectors_to_search, limit=3, output_fields=["title"])
for hits in result:
    for hit in hits:
        print(f"hit: {hit}")
...
field_index_names = plasmod_client.list_indexes(collection_name, field_name = "embeddings")
print(f"index names for {collection_name}`s field embeddings:", field_index_names)

try:
    plasmod_client.drop_index(collection_name, "my_trie")
